#!/bin/bash
#═══════════════════════════════════════════════════════════════════════════
# BOLT.DIY NBILITY - Installation Script v4.5
# Clone complet depuis GitHub repository + Fix Wrangler PATH
...
if [ "$EUID" -en 0 ]; then 
    ...
    echo "  sudo ./install_bolt_nbility_v4.5.sh"
    echo ""
...
print_banner() {
    ...
    echo "              ╚═══════════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    echo -e "${MAGENTA}${BOLD}                                    Installation Interactive v4.5${NC}"
...
check_internet_and_github() {
    ...
    print_step "Test de connectivité Internet (Google DNS)..."
    if ! ping -c 1 -w 5 8.8.8.8 &> /dev/null; then
        print_error "Pas de connexion Internet"
        print_error "Vérifiez votre connexion réseau et réessayez"
        exit 1
    fi
    print_success "Connexion Internet OK"
    
    print_step "Test de connectivité à GitHub..."
    if ! ping -c 1 -w 5 github.com &> /dev/null; then
        print_error "Impossible de joindre GitHub"
        print_error "Vérifiez votre connexion Internet et les paramètres DNS"
        exit 1
    fi
    print_success "Connexion à GitHub OK"
    
    print_step "Vérification du repository NBILITY-HOME/BOLT.DIY-INTRANET..."
    if ! git ls-remote "$REPO_URL" &> /dev/null; then
        print_error "Impossible d'accéder au repository"
        print_error "URL: $REPO_URL"
        print_info "Vérifiez que le repository existe et est accessible"
        exit 1
    fi
    print_success "Repository accessible"
    
    echo ""
    print_success "Toutes les vérifications de connectivité sont OK"
    echo ""
...
check_prerequisites() {
    ...
    if command -v docker &> /dev/null; then
        ...
        if ! docker ps &> /dev/null; then
            print_error "Docker nécessite sudo"
            print_info "Ajoutez votre utilisateur au groupe docker:"
...
fix_bolt_dockerfile() {
    print_banner
    print_section "APPLICATION DU FIX WRANGLER PATH"
    
    print_step "Vérification du Dockerfile de bolt.diy..."
    
    if [ ! -f "$BOLT_DIR/Dockerfile" ]; then
        print_error "Dockerfile non trouvé dans $BOLT_DIR"
        exit 1
    fi
    ...
    if grep -q 'ENV PATH="/app/node_modules/.bin:\${PATH}"' "$BOLT_DIR/Dockerfile"; then
        print_info "Le fix PATH est déjà appliqué dans le Dockerfile"
    else
        print_step "Application du fix PATH dans le Dockerfile..."
        
        # Sauvegarder l'original
        cp "$BOLT_DIR/Dockerfile" "$BOLT_DIR/Dockerfile.backup"
        print_info "Sauvegarde créée: Dockerfile.backup"
        
        # Ajouter la ligne PATH après ENV NODE_ENV=production
        sed -i '/^ENV NODE_ENV=production$/a # FIX NBILITY: Add node_modules/.bin to PATH so wrangler is accessible\nENV PATH="/app/node_modules/.bin:${PATH}"' "$BOLT_DIR/Dockerfile"
        
        print_success "Fix PATH appliqué dans le Dockerfile"
    fi
...
DOCKERCOMPOSE
    ...
    echo ""
    print_success "Fix Wrangler PATH appliqué avec succès"
    print_info "Le conteneur bolt-nbility-core sera construit localement"
    echo ""
    echo -e "${GREEN}${BOLD}Appuyez sur ENTRÉE pour continuer...${NC}"
    read
}
...
ENVFILE
    ...
    print_info "Construction depuis le Dockerfile local de bolt.diy (cela peut prendre plusieurs minutes)..."
